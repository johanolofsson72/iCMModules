...
Public Class Timeline : Inherits clsSiteModuleControl
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        scriptString = "<script language=JavaScript> function OPENEDIT" & ModuleId & "(CalId) {window.open('Desktop/Modules/Timeline/Appointment.aspx?PageId=" & ModuleConfiguration.PageId & "&ModId=" & ModuleId & "&CalId='+CalId,'Appointment','width=600,height=400')}</script>"
        ...
        End If
        scriptString = "<script language=JavaScript> function OPENNEW" & ModuleId & "(date) {window.open('Desktop/Modules/Timeline/Appointment.aspx?PageId=" & ModuleConfiguration.PageId & "&ModId=" & ModuleId & "&SelectedDate='+date,'Appointment','width=600,height=400')}</script>"
    ...
    Private Sub LoadCalendar(ByVal SelectedDate As Date, ByVal SelectedDates As Integer)
        Try
            ...
            Bar2.Text = oCal.GetTimelineWeekBar(CalendarHolder, SelectedDate, SelectedDates)
            TimeHolder.Controls.Add(Bar2)

            '''' Loop the dsCalendar Tables (TimeSpan, Appointments, etc)...
            '''dsCalendar = oCal.GetCalendarField(SelectedDate, True, SelectedDates)
            ...
            '''    dsCalendar = oCal.GetCalendarField(SelectedDate.AddDays(i), False, SelectedDates)
            '''    For Each dt In dsCalendar.Tables
            '''        TimeSpan = New System.Web.UI.WebControls.Literal
            '''        For Each dr In dt.Rows
            '''            TimeSpan.Text += dr("value") & vbCrLf

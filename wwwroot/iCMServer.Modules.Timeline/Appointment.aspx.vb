            ...
            If (CType(sDate, Date) <= CType(sDate2, Date)) Then
                If CalId = 0 Then
                    ...
                    If Equals(CType(sDate, Date), CType(sDate2, Date)) Then
                        Parent = oCal.AddAppointment(0, txtSubject.Text, txtText.Text, ddLabel.SelectedValue, sDate, sDate2, "00:00", "23:59")
                    Else
                        ' Skapa inlägg, ett för varje dag...
                        Dim eDate As System.TimeSpan = CType(sDate2, Date).Subtract(CType(sDate, Date))
                        Parent = oCal.AddAppointment(0, txtSubject.Text, txtText.Text, ddLabel.SelectedValue, sDate, sDate, "00:00", "23:59")
                        For d As Integer = 1 To eDate.Days
                            Try
                                Dim none As Integer = oCal.AddAppointment(Parent, txtSubject.Text, txtText.Text, ddLabel.SelectedValue, CType(sDate, Date).AddDays(d).ToString, CType(sDate, Date).AddDays(d).ToString, "00:00", "23:59")
                            Catch ex As Exception

...
#End Region
    ...
    Private Sub BindData()
        ...
        lblUsers.Text = Server.HtmlDecode(oLang.Find(al, "calendar_users")) & "&nbsp;"
        ...
        Else
                ...
                If ds.Tables(0).Rows.Count > 1 Then
                    ...
                    sUser = CType(dr("cal_attendees"), String)
                Else
                    Dim dr As DataRow = ds.Tables(0).Rows(0)
                    Call RenderDateBoxes(dr("cal_starttime"))
                    Dim p1 As String = CType(dr("cal_starttime"), Date).ToLongTimeString
            ...
            If (CType(sDate, Date) <= CType(sDate2, Date)) Then
                If CalId = 0 Then
                    ...
                    If Equals(CType(sDate, Date), CType(sDate2, Date)) Then
                        Parent = oCal.AddAppointment(0, txtSubject.Text, txtText.Text, ddLabel.SelectedValue, sDate, sDate2, "00:00", "23:59", ddUsers.SelectedValue)
                    Else
                        ' Skapa inlägg, ett för varje dag...
                        Dim eDate As System.TimeSpan = CType(sDate2, Date).Subtract(CType(sDate, Date))
                        Parent = oCal.AddAppointment(0, txtSubject.Text, txtText.Text, ddLabel.SelectedValue, sDate, sDate, "00:00", "23:59", ddUsers.SelectedValue)
                        For d As Integer = 1 To eDate.Days
                            Try
                                Dim none As Integer = oCal.AddAppointment(Parent, txtSubject.Text, txtText.Text, ddLabel.SelectedValue, CType(sDate, Date).AddDays(d).ToString, CType(sDate, Date).AddDays(d).ToString, "00:00", "23:59", ddUsers.SelectedValue)
                            Catch ex As Exception
                ...
                Else
                    ...
                    End If
                End If
                Dim strParentUrl As String = Left(Request.Url.ToString, InStr(LCase(Request.Url.ToString), "desktop/modules/timesheet/appointment.aspx") - 1) & "iCM.aspx?PageId=" & PageId
                ViewState.Item("iCMServer.Modules.Timesheet.SelectedDay" & ModId) = sDate

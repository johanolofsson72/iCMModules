...
Public Class Timesheet2 : Inherits clsSiteModuleControl
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        SelectedDay = ViewState.Item("iCMServer.Modules.Timesheet2.SelectedDay" & ModuleId)
        ...
        scriptString = "<script language=JavaScript> function OPENEDIT" & ModuleId & "(CalId) {window.open('Desktop/Modules/Timesheet2/Appointment2.aspx?PageId=" & ModuleConfiguration.PageId & "&ModId=" & ModuleId & "&CalId='+CalId,'Appointment','width=600,height=420')}</script>"
        ...
        End If
        scriptString = "<script language=JavaScript> function OPENNEW" & ModuleId & "(date) {window.open('Desktop/Modules/Timesheet2/Appointment2.aspx?PageId=" & ModuleConfiguration.PageId & "&ModId=" & ModuleId & "&SelectedDate='+date,'Appointment','width=600,height=420')}</script>"
        ...
        End If
        scriptString = "<script language=JavaScript> function OPENNEWX" & ModuleId & "(datum, user) {window.open('Desktop/Modules/Timesheet2/Appointment2.aspx?PageId=" & ModuleConfiguration.PageId & "&X='+datum+'" & "&UsrId='+user+'" & "&ModId=" & ModuleId & "&SelectedDate='+datum,'Appointment','width=600,height=420')}</script>"
    ...
    Private Sub SetCulture()
        ...
        Dim lang As String = Server.HtmlDecode(oLang.Find(al, "timesheet2_lang"))
        Select Case lang
            Case "english"
    ...
    Private Sub BindData()
        ...
        imgNewAppointment.AlternateText = Server.HtmlDecode(oLang.Find(al, "timesheet2_newapp"))
        imgNewAppointment.ToolTip = Server.HtmlDecode(oLang.Find(al, "timesheet2_newapp"))
        lnkNew.Text = Server.HtmlDecode(oLang.Find(al, "timesheet2_new"))
        lnkNew.ToolTip = Server.HtmlDecode(oLang.Find(al, "timesheet2_newapp"))
        ...
        ViewState.Item("iCMServer.Modules.Timesheet2.SelectedDay" & ModuleId) = cal.SelectedDate

    End Sub

    Private Sub lnkNew_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkNew.Click
        Response.Write("<SCRIPT LANGUAGE=javaScript>var Appointment = window.open('Desktop/Modules/Timesheet2/Appointment2.aspx?PageId=" & ModuleConfiguration.PageId & "&ModId=" & ModuleId & "&SelectedDate=" & cal.SelectedDate & "','Appointment','width=600,height=400')</SCRIPT>")
        LoadCalendar(cal.SelectedDate, cal.SelectedDates.Count)
    End Sub
    ...
    Private Sub tim_SelectionChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cal.SelectionChanged
        LoadCalendar(cal.SelectedDate, cal.SelectedDates.Count)
        'ViewState.Item("iCMServer.Modules.Timesheet2.SelectedDay") = cal.SelectedDate
    End Sub

    Private Sub LoadCalendar(ByVal SelectedDate As Date, ByVal SelectedDates As Integer)
        Try
            Dim oCal As New clsTimesheet2(ModuleId)
            ...
            Bar.Text = oCal.GetTimesheet2MonthBar(SelectedDate, SelectedDates)
            BarHolder.Controls.Add(Bar)
            Dim Bar2 As New System.Web.UI.WebControls.Literal
            Bar2.Text = oCal.GetTimesheet2WeekBar(CalendarHolder, SelectedDate, SelectedDates)
    ...
    Private Sub tim_VisibleMonthChanged(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.MonthChangedEventArgs) Handles cal.VisibleMonthChanged
        LoadCalendar(cal.SelectedDate, cal.SelectedDates.Count)
        'ViewState.Item("iCMServer.Modules.Timesheet2.SelectedDay" & ModuleId) = cal.SelectedDate
    End Sub

    Private Sub tim_DayRender(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DayRenderEventArgs) Handles cal.DayRender
        If Not IsFirstDay Then
            If Not e.Day.IsOtherMonth Then
                ...
                For Each dr As DataRow In AllDays.Tables(0).Rows
                    If CType(dr("tim_endtime"), Date).Month = e.Day.Date.Month And CType(dr("tim_endtime"), Date).Day = e.Day.Date.Day Then

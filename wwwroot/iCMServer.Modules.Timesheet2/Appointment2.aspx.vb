...
#End Region
    ...
    Private Sub BindData()
        ...
        al = oLang.GetLanguageTextByLocation(oSite.Language, "iConsulting.iCMServer.Modules.Timesheet2")

        lblHeader.Text = IIf(CalId = 0, Server.HtmlDecode(oLang.Find(al, "timesheet2_newapp")), Server.HtmlDecode(oLang.Find(al, "timesheet2_newedit"))) '"Appointment" 'Server.HtmlDecode(oLang.Find(al, "timesheet2_newapp"))'"Appointment" 'Server.HtmlDecode(oLang.Find(al, "timesheet2_newedit"))
        lblSubject.Text = Server.HtmlDecode(oLang.Find(al, "timesheet2_subject")) & "&nbsp;"
        lblText.Text = Server.HtmlDecode(oLang.Find(al, "timesheet2_text")) & "&nbsp;"
        lblStartTime.Text = Server.HtmlDecode(oLang.Find(al, "timesheet2_datefrom")) & "&nbsp;"
        lblEndTime.Text = Server.HtmlDecode(oLang.Find(al, "timesheet2_dateto")) & "&nbsp;"
        'lblStart.Text = Server.HtmlDecode(oLang.Find(al, "timesheet2_start")) & "&nbsp;"
        'lblEnd.Text = Server.HtmlDecode(oLang.Find(al, "timesheet2_end")) & "&nbsp;"
        lblLabel.Text = Server.HtmlDecode(oLang.Find(al, "timesheet2_label")) & "&nbsp;"
        lblUsers.Text = Server.HtmlDecode(oLang.Find(al, "timesheet2_users")) & "&nbsp;"
        btnUpdate.Text = Server.HtmlDecode(oLang.Find(al, "timesheet2_update"))
        btnCancel.Text = Server.HtmlDecode(oLang.Find(al, "timesheet2_cancel"))
        btnDelete.Text = Server.HtmlDecode(oLang.Find(al, "timesheet2_delete"))
        lblError.Text = Server.HtmlDecode(oLang.Find(al, "timesheet2_errordate"))
        lblError.Visible = False 'timesheet2_text
        lblError2.Text = Server.HtmlDecode(oLang.Find(al, "timesheet2_errordate"))
        lblError2.Visible = False 'timesheet2_text

        Dim sConfirm As String = Server.HtmlDecode(oLang.Find(al, "timesheet2_confirmdelete"))
        btnDelete.Attributes.Add("onclick", "return confirm('" & sConfirm & "');")

        ddLabel.Items.Add(New ListItem(Server.HtmlDecode(oLang.Find(al, "timesheet2_label_none")), "#FFFFFF"))
        ddLabel.Items.Add(New ListItem(Server.HtmlDecode(oLang.Find(al, "timesheet2_label_important")), "#FF9999"))
        ddLabel.Items.Add(New ListItem(Server.HtmlDecode(oLang.Find(al, "timesheet2_label_business")), "#6699CC"))
        ddLabel.Items.Add(New ListItem(Server.HtmlDecode(oLang.Find(al, "timesheet2_label_personal")), "#99CC66"))
        ddLabel.Items.Add(New ListItem(Server.HtmlDecode(oLang.Find(al, "timesheet2_label_vacation")), "#CCCCCC"))
        ddLabel.Items.Add(New ListItem(Server.HtmlDecode(oLang.Find(al, "timesheet2_label_mustattend")), "#FF9966"))
        ddLabel.Items.Add(New ListItem(Server.HtmlDecode(oLang.Find(al, "timesheet2_label_travelrequired")), "#99FFFF"))
        ddLabel.Items.Add(New ListItem(Server.HtmlDecode(oLang.Find(al, "timesheet2_label_needspreperation")), "#CCCC99"))
        ddLabel.Items.Add(New ListItem(Server.HtmlDecode(oLang.Find(al, "timesheet2_label_birthday")), "#CC99FF"))
        ddLabel.Items.Add(New ListItem(Server.HtmlDecode(oLang.Find(al, "timesheet2_label_anniversary")), "#99CCCC"))
        ddLabel.Items.Add(New ListItem(Server.HtmlDecode(oLang.Find(al, "timesheet2_label_phonecall")), "#FFCC66"))
        ...
        If CalId = 0 Then
            ...
            ' /// Users
            Dim oTimesheet2 As New clsTimesheet2(ModId)
            Dim dsUserRoles As New System.data.DataSet
            dsUserRoles = oTimesheet2.GetAllUsers
            Dim l As ListItem
            Dim dra As DataRow
            ddUsers.Items.Clear()
            Dim Users() As String
        ...
        Else
            Dim oCal As New clsTimesheet2(ModId)
            ...
            Else
                ...
                sStartDay = Format(CType(dr("tim_starttime"), Date).Day, "00")
                sStartTime = Format(CType(p1, Date).Hour, "00") & ":" & Format(CType(p1, Date).Minute, "00")
                sEndYear = CType(dr("tim_starttime"), Date).Year.ToString
                sEndMonth = Format(CType(dr("tim_starttime"), Date).Month, "00")
                sEndDay = Format(CType(dr("tim_starttime"), Date).Day, "00")
                sEndTime = Format(CType(p2, Date).Hour, "00") & ":" & Format(CType(p2, Date).Minute, "00")
                sLabel = CType(dr("tim_label"), String)
                sUser = CType(dr("tim_attendees"), String)
            End If
            ...
            If (CType(sDate, Date) <= CType(sDate2, Date)) Then
                If CalId = 0 Then
                    ...
                    If Equals(CType(sDate, Date), CType(sDate2, Date)) Then
                        Parent = oCal.AddAppointment(0, Server.HtmlEncode(txtSubject.Text), Server.HtmlEncode(txtText.Text), ddLabel.SelectedValue, sDate, sDate2, "00:00", "23:59", ddUsers.SelectedValue)
                    Else
                        ' Skapa inlägg, ett för varje dag...
                        Dim eDate As System.TimeSpan = CType(sDate2, Date).Subtract(CType(sDate, Date))
                        Parent = oCal.AddAppointment(0, Server.HtmlEncode(txtSubject.Text), Server.HtmlEncode(txtText.Text), ddLabel.SelectedValue, sDate, sDate, "00:00", "23:59", ddUsers.SelectedValue)
                        For d As Integer = 1 To eDate.Days
                            Try
                                Dim none As Integer = oCal.AddAppointment(Parent, Server.HtmlEncode(txtSubject.Text), Server.HtmlEncode(txtText.Text), ddLabel.SelectedValue, CType(sDate, Date).AddDays(d).ToString, CType(sDate, Date).AddDays(d).ToString, "00:00", "23:59", ddUsers.SelectedValue)
                            Catch ex As Exception
                ...
                If sUrl.IndexOf("Desktop/Modules/Timesheet2/Appointment2.aspx") > 0 Then
                    Dim strParentUrl As String = Left(sUrl, InStr(sUrl, "Desktop/Modules/Timesheet2/Appointment2.aspx") - 1) & "iCM.aspx?PageId=" & PageId
                    ViewState.Item("iCMServer.Modules.Timesheet2.SelectedDay" & ModId) = sDate
    ...
    Private Sub btnDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        End If
        Dim strParentUrl As String = Left(Request.Url.ToString, InStr(LCase(Request.Url.ToString), "desktop/modules/timesheet2/appointment2.aspx") - 1) & "iCM.aspx?PageId=" & PageId
        ViewState.Item("iCMServer.Modules.Timesheet2.SelectedDay" & ModId) = sDate
        Response.Write("<SCRIPT LANGUAGE=javaScript>window.opener.location.href='" & strParentUrl & "'; this.close();</script>")
    End Sub

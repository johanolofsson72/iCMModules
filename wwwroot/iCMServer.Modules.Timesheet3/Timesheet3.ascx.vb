...
Public Class Timesheet3 : Inherits clsSiteModuleControl
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        SelectedDay = ViewState.Item("iCMServer.Modules.Timesheet3.SelectedDay" & ModuleId)
        ...
        scriptString = "<script language=JavaScript> function OPENEDIT" & ModuleId & "(CalId) {window.open('Desktop/Modules/Timesheet3/Appointment3.aspx?PageId=" & ModuleConfiguration.PageId & "&ModId=" & ModuleId & "&CalId='+CalId,'Appointment','width=600,height=420')}</script>"
        ...
        End If
        scriptString = "<script language=JavaScript> function OPENNEW" & ModuleId & "(date) {window.open('Desktop/Modules/Timesheet3/Appointment3.aspx?PageId=" & ModuleConfiguration.PageId & "&ModId=" & ModuleId & "&SelectedDate='+date,'Appointment','width=600,height=420')}</script>"
        ...
        End If
        scriptString = "<script language=JavaScript> function OPENNEWX" & ModuleId & "(datum, user) {window.open('Desktop/Modules/Timesheet3/Appointment3.aspx?PageId=" & ModuleConfiguration.PageId & "&X='+datum+'" & "&UsrId='+user+'" & "&ModId=" & ModuleId & "&SelectedDate='+datum,'Appointment','width=600,height=420')}</script>"
    ...
    Private Sub SetCulture()
        ...
        Dim lang As String = Server.HtmlDecode(oLang.Find(al, "timesheet3_lang"))
        Select Case lang
            Case "english"
    ...
    Private Sub BindData()
        ...
        imgNewAppointment.AlternateText = Server.HtmlDecode(oLang.Find(al, "timesheet3_newapp"))
        imgNewAppointment.ToolTip = Server.HtmlDecode(oLang.Find(al, "timesheet3_newapp"))
        lnkNew.Text = Server.HtmlDecode(oLang.Find(al, "timesheet3_new"))
        lnkNew.ToolTip = Server.HtmlDecode(oLang.Find(al, "timesheet3_newapp"))
        ...
        ViewState.Item("iCMServer.Modules.Timesheet3.SelectedDay" & ModuleId) = cal.SelectedDate

    End Sub

    Private Sub lnkNew_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkNew.Click
        Response.Write("<SCRIPT LANGUAGE=javaScript>var Appointment = window.open('Desktop/Modules/Timesheet3/Appointment3.aspx?PageId=" & ModuleConfiguration.PageId & "&ModId=" & ModuleId & "&SelectedDate=" & cal.SelectedDate & "','Appointment','width=600,height=400')</SCRIPT>")
        LoadCalendar(cal.SelectedDate, cal.SelectedDates.Count)
    End Sub
    ...
    Private Sub tim_SelectionChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cal.SelectionChanged
        LoadCalendar(cal.SelectedDate, cal.SelectedDates.Count)
        'ViewState.Item("iCMServer.Modules.Timesheet3.SelectedDay") = cal.SelectedDate
    End Sub

    Private Sub LoadCalendar(ByVal SelectedDate As Date, ByVal SelectedDates As Integer)
        Try
            Dim oCal As New clsTimesheet3(ModuleId)
            ...
            Bar.Text = oCal.GetTimesheet3MonthBar(SelectedDate, SelectedDates)
            BarHolder.Controls.Add(Bar)
            Dim Bar2 As New System.Web.UI.WebControls.Literal
            Bar2.Text = oCal.GetTimesheet3WeekBar(CalendarHolder, SelectedDate, SelectedDates)
    ...
    Private Sub tim_DayRender(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DayRenderEventArgs) Handles cal.DayRender
        If Not e.Day.IsOtherMonth Then
            Dim oCal As New clsTimesheet3(ModuleId)
            AllDays = oCal.GetAllAppointmentsInMonthTimeline3(e.Day.Date.Year.ToString & "-" & Format(e.Day.Date.Month, "00") & "-01")
            IsFirstDay = True
            For Each dr As DataRow In AllDays.Tables(0).Rows
                If e.Day.Date >= CType(dr("tim_starttime"), Date) And _
                   e.Day.Date <= CType(dr("tim_endtime"), Date) Then

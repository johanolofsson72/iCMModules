...
#Region " Public Functions "
    ...
    Public Function AddAppointment(ByVal Parent As Integer, ByVal Subject As String, ByVal Text As String, ByVal Label As String, ByVal StartDate As String, ByVal EndDate As String, ByVal StartTime As String, ByVal EndTime As String, ByVal User As String) As Integer
        ...
        Try
            ...
            End If
            If Not oDO.GetEmptyDataSet("tim_timesheet3", "", ED, EC, ds) Then
    ...
    Public Function UpdateAppointment(ByVal Parent As Integer, ByVal CalId As Integer, ByVal Subject As String, ByVal Text As String, ByVal Label As String, ByVal StartDate As String, ByVal EndDate As String, ByVal StartTime As String, ByVal EndTime As String) As Integer
        ...
        Try
            ...
            If CType(sStartDate, Date) = CType(sEndDate, Date) Or CType(sStartDate, Date) > CType(sEndDate, Date) Then
                sEndDate = CType(sStartDate, Date).AddMinutes(30).ToString
            End If
            If Not oDO.GetDataSet("tim_timesheet3", "sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_id = " & CalId, "", "", ED, EC, ds) Then
    ...
    Public Function GetAppointment(ByVal CalId As Integer) As DataSet
        Dim ds As New DataSet
        Try
            If Not oDO.GetDataSet("tim_timesheet3", "sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_id = " & CalId & " OR sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_allday = " & CalId, "tim_id", "", ED, EC, ds) Then

            End If
            If Not ds.Tables(0).Rows(0)("tim_allday") = 0 Then
                Dim Parent As Integer = ds.Tables(0).Rows(0)("tim_allday")
                ds = New DataSet
                If Not oDO.GetDataSet("tim_timesheet3", "sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_id = " & Parent & " OR sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_allday = " & Parent, "tim_id", "", ED, EC, ds) Then
    ...
    Public Function DeleteAppointment(ByVal CalId As Integer) As Boolean
        ...
        Try
            If Not oDO.GetDataSet("tim_timesheet3", "sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_id = " & CalId & " OR sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_allday = " & CalId, "tim_id", "", ED, EC, dsParent) Then
            ...
            If Not Parent = 0 Then
                If Not oDO.GetDataSet("tim_timesheet3", "sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_id = " & Parent & " OR sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_allday = " & Parent, "tim_id", "", ED, EC, dsChildren) Then
    ...
    Public Function GetAllAppointmentsInMonth(ByVal StartDate As String, ByVal EndDate As String) As DataSet
        ...
        Try
            ...
            If Not oDO.GetDefinedDataSet("tim_timesheet3", oDefine.DataSet, "sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_starttime >= '" & StartDate & "' AND tim_endtime <= '" & EndDate & "' AND tim_hidden = 0 AND tim_deleted = 0", "", "", ED, EC, ds) Then
...
#Region " Private Render Functions "
    ...
    Private Function GetAppointments(ByVal SelectedDate As Date) As DataSet
        ...
        Try
            ...
            If Not oDO.GetDataSet("tim_timesheet3", "sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_starttime >= ' " & StartDate & " '" & " AND tim_endtime <= ' " & EndDate & " ' AND tim_hidden = 0 AND tim_deleted = 0", "tim_starttime", "", ED, EC, ds) Then
                'If Not oDO.GetDataSet("tim_timesheet3", "sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_starttime >= ' " & StartDate & " '" & " AND tim_starttime <= ' " & EndDate & " ' AND tim_hidden = 0 AND tim_deleted = 0", "tim_starttime", "", ED, EC, ds) Then
            End If
            Return ds
        Catch ex As Exception
            Return New DataSet
...
#Region " Timeline Functions "
    ...
    Private Function GetTimelineAppointments(ByVal SelectedDate As Date, ByVal Span As Integer) As DataSet
        ...
        Try
            ...
            If Not oDO.GetDataSet("tim_timesheet3", "sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_endtime <= ' " & sEndDate & " ' AND tim_endtime >= ' " & sStartDate & " ' AND tim_hidden = 0 AND tim_deleted = 0", "tim_allday, tim_starttime", "", ED, EC, ds) Then
                'If Not oDO.GetDataSet("tim_timesheet3", "sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_starttime >= ' " & sStartDate & " '" & " AND tim_endtime <= ' " & sEndDate & " ' AND tim_hidden = 0 AND tim_deleted = 0", "tim_allday, tim_starttime", "", ED, EC, ds) Then
            End If
            Return ds
        Catch ex As Exception
            Return New DataSet
...
#Region " Timesheet3 Functions "

    Public Function GetAllAppointmentsInMonthTimeline3(ByVal StartDate As String) As DataSet
        Dim ds As New DataSet
        Try
            StartDate = CType(StartDate, DateTime).AddDays(-1)
            Dim EndDate As String = CType(StartDate, DateTime).AddDays(2 + CType(StartDate, DateTime).DaysInMonth(CType(StartDate, DateTime).Year, CType(StartDate, DateTime).Month))
            ...
            oDefine.AddDefinedTableColumn("tim_starttime")
            oDefine.AddDefinedTableColumn("tim_endtime")
            If Not oDO.GetDefinedDataSet("tim_timesheet3", oDefine.DataSet, "sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_endtime > '" & StartDate & "' AND tim_starttime < '" & EndDate & "' AND tim_hidden = 0 AND tim_deleted = 0 OR sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_endtime > '" & StartDate & "' AND tim_starttime < '" & EndDate & "' AND tim_hidden = 0 AND tim_deleted = 0", "", "", ED, EC, ds) Then
    ...
    Private Function GetTimelineAppointments3(ByVal SelectedDate As Date, ByVal Span As Integer) As DataSet
        Dim ds As New DataSet
        Try
            Dim Days As Integer = SelectedDate.DaysInMonth(SelectedDate.Year, SelectedDate.Month)
            Dim StartDate As String = SelectedDate.Year.ToString & "-" & SelectedDate.Month.ToString & "-" & SelectedDate.Day.ToString
            Dim EndDate As String = SelectedDate.AddDays(Span).Year.ToString & "-" & SelectedDate.AddDays(Span).Month.ToString & "-" & SelectedDate.AddDays(Span).Day.ToString
            If Not oDO.GetDataSet("tim_timesheet3", "sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_endtime > '" & StartDate & "' AND tim_starttime < '" & EndDate & "' AND tim_hidden = 0 AND tim_deleted = 0 OR sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_endtime > '" & StartDate & "' AND tim_starttime < '" & EndDate & "' AND tim_hidden = 0 AND tim_deleted = 0", "tim_allday, tim_starttime", "", ED, EC, ds) Then
    ...
    Public Function AddAppointment3(ByVal Subject As String, ByVal Text As String, ByVal Label As String, ByVal StartDate As String, ByVal EndDate As String, ByVal StartTime As String, ByVal EndTime As String, ByVal User As String) As Integer
        Dim ds As New DataSet
        Try
            Dim sStartDate As String = StartDate & " " & StartTime
            Dim sEndDate As String = EndDate & " " & EndTime
            If Not oDO.GetEmptyDataSet("tim_timesheet3", "", ED, EC, ds) Then
    ...
    Public Function UpdateAppointment3(ByVal TimId As Integer, ByVal Subject As String, ByVal Text As String, ByVal Label As String, ByVal StartDate As String, ByVal EndDate As String, ByVal StartTime As String, ByVal EndTime As String, ByVal User As String) As Integer
        Dim ds As New DataSet
        Try
            Dim sStartDate As String = StartDate & " " & StartTime
            Dim sEndDate As String = EndDate & " " & EndTime
            If CType(sStartDate, Date) = CType(sEndDate, Date) Or CType(sStartDate, Date) > CType(sEndDate, Date) Then
                sEndDate = CType(sStartDate, Date).AddMinutes(30).ToString
            End If
            If Not oDO.GetDataSet("tim_timesheet3", "sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_id = " & TimId, "", "", ED, EC, ds) Then
    ...
    Public Function DeleteAppointment3(ByVal TimId As Integer) As Integer
        Dim ds As New DataSet
        Try
            If Not oDO.GetDataSet("tim_timesheet3", "sit_id = " & oSite.SiteId & " AND pag_id = " & oSite.ActivePage.PageId & " AND mod_id = " & iModId & " AND tim_id = " & TimId, "", "", ED, EC, ds) Then
            ...
            End If
            If oSite.SiteSoftDelete Then
                ds.Tables(0).Rows(0)("tim_deleted") = 1
            Else
    ...
    Private Function GetWeek3(ByVal SelectedDate As Date) As String
        Try
            If System.Threading.Thread.CurrentThread.CurrentCulture.Name = "en-US" Then
                Return "Week " & CType(DatePart(DateInterval.WeekOfYear, SelectedDate, Microsoft.VisualBasic.FirstDayOfWeek.System, FirstWeekOfYear.System), String)
            Else
    ...
    Private Function GetWeekNumber3(ByVal SelectedDate As Date) As String
        Try
            Return CType(DatePart(DateInterval.WeekOfYear, SelectedDate, Microsoft.VisualBasic.FirstDayOfWeek.System, FirstWeekOfYear.System), String)
        Catch ex As Exception
            Return CType(DatePart(DateInterval.WeekOfYear, Now, Microsoft.VisualBasic.FirstDayOfWeek.System, FirstWeekOfYear.System), String)
    ...
    Public Function GetTimesheet3MonthBar(ByVal SelectedDate As Date, ByVal SelectedDates As Integer) As String
        Try
            Dim sHtml As String = String.Empty
            sHtml += "<table height=""23px"" width=""100%"" border=""0"" cellSpacing=""0"" cellPadding=""0"" align=""left"" style=""BORDER-TOP: #000000 0px solid;BORDER-BOTTOM: #000000 2px solid;BORDER-LEFT: #000000 0px solid;BORDER-RIGHT: #000000 0px solid;"">"
            sHtml += "  <tr>"
    ...
    Public Function GetTimesheet3WeekBar(ByRef c As System.web.ui.Control, ByVal SelectedDate As Date, ByVal SelectedDates As Integer) As String
        ...
        Try
            ...
            ' /// Users
            Dim dsUsers As DataSet = GetAllUsers3()
            Dim dsApp As DataSet = GetTimelineAppointments3(SelectedDate.AddDays(0), 6)
            Dim Users() As String
            For Each _mod As clsModuleSettings In oSite.ActivePage.Modules
                If _mod.ModuleId = Me.iModId Then Users = Split(_mod.EditSrc, ";") : Exit For
            Next
            For Each s As String In Users
                For Each dru As DataRow In dsUsers.Tables(0).Rows
                    If s.Length > 0 And CType(dru("usr_id"), String).ToLower = s.ToLower Then
                        Dim Max As Integer = CalculateMaxRowsOnUser3(dsApp, SelectedDate, s)
                        Dim First As String = IIf(IsDBNull(dru("usr_firstname")), "", dru("usr_firstname"))
                        Dim Second As String = IIf(IsDBNull(dru("usr_lastname")), "", dru("usr_lastname"))
                        If Second.Length > 1 Then
                            Second = Second.Substring(0, 2)
                        ...
                        sHtml += "<tr>"
                        sHtml += "<td valign=""top"" align=""left"" width=""100px""" & """ style=""BORDER-LEFT: #000000 0px solid;BORDER-BOTTOM: #000000 0px solid;FONT-WEIGHT: normal; FONT-SIZE: xx-small; FONT-FAMILY: Verdana"">" & First & "&nbsp;" & Second & "&nbsp;</td>"
                        sHtml += CreateAppointmentHolder3(dsApp, SelectedDate.AddDays(0), s, Width, Max)
                        sHtml += CreateAppointmentHolder3(dsApp, SelectedDate.AddDays(1), s, Width, Max)
                        sHtml += CreateAppointmentHolder3(dsApp, SelectedDate.AddDays(2), s, Width, Max)
                        sHtml += CreateAppointmentHolder3(dsApp, SelectedDate.AddDays(3), s, Width, Max)
                        sHtml += CreateAppointmentHolder3(dsApp, SelectedDate.AddDays(4), s, Width, Max)
                        sHtml += "</tr>"
                    End If
                Next
            Next
    ...
    Private Function CalculateMaxRowsOnUser3(ByVal dsApp As DataSet, ByVal SelectedDate As Date, ByVal UsrId As String) As Integer
        Try
            Dim Max As Integer = 0
            Dim tMax As Integer = 0
            tMax = 0
    ...
    Private Function CreateAppointmentHolder3(ByVal dsApp As DataSet, ByVal SelectedDate As Date, ByVal UsrId As String, ByVal iWidth As Integer, ByVal Max As Integer) As String
        Dim sHtml As String
        Dim sHtmlStart As String
        Dim Exist As Boolean = False
        Dim tMax As Integer = 0
        Try
            sHtml += "<table border=""0"" cellSpacing=""0"" cellPadding=""0"" width=""100%"" >"
            For Each drt As DataRow In dsApp.Tables(0).Rows
                If CType(drt("tim_attendees"), String) = UsrId And CType(drt("tim_starttime"), Date).ToShortDateString = SelectedDate.ToShortDateString Or _
                    CType(drt("tim_attendees"), String) = UsrId And CType(drt("tim_endtime"), Date).ToShortDateString = SelectedDate.ToShortDateString Or _
                    CType(drt("tim_attendees"), String) = UsrId And CType(drt("tim_starttime"), Date).ToShortDateString < SelectedDate.ToShortDateString And CType(drt("tim_endtime"), Date).ToShortDateString > SelectedDate.ToShortDateString Then
                    Exist = True
                    tMax += 1
                    sHtml += "<tr height=""20"">"
                    sHtml += " <td height=""20"" " & IIf(m_IsAuthorized, " onclick=""OPENEDIT" & iModId & "(" & CType(drt("tim_id"), Integer) & ");""", "") & " style=""BORDER-BOTTOM: #000000 " & IIf(tMax < Max, "1", "0") & "px solid;" & IIf(m_IsAuthorized, "cursor:hand;""", "") & """ title=""" & drt("tim_subject") & vbCrLf & drt("tim_text") & """ align=""left"" width=""" & iWidth & "%"" bgcolor=""" & CType(drt("tim_label"), String) & """>"
    ...
    Public Function GetAllUsers3() As DataSet
        Dim dsRoles As New DataSet
        Dim sError As String
        If Not oDO.GetDataSet("usr_users", "usr_hidden = 0 AND usr_deleted = 0 AND sit_id = " & oSite.SiteId, "usr_firstname, usr_lastname, usr_email", sError, ED, EC, dsRoles) Then
            System.Diagnostics.Debug.WriteLine(sError)
    ...
    Public Function UpdateModuleUsers3(ByVal PagId As Integer, ByVal ModId As Integer, ByVal Users As String) As Boolean
        Dim ds As New DataSet
        Try
            If Not oDO.GetDataSet("mod_modules", "sit_id = " & oSite.SiteId & " AND pag_id = " & PagId & " AND mod_id = " & ModId, "", "", ED, EC, ds) Then
